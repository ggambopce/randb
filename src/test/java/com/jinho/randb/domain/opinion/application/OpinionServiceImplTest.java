...
class OpinionServiceImplTest {
    ...
    @Test
    @DisplayName("게시글 ID로 의견 찾기 테스트")
    void findByPostIdTest() {
        when(opinionRepository.findByPostId(anyLong())).thenReturn(List.of(opinion));

        List<Opinion> opinions = opinionService.findByPostId(1L);

        assertEquals(1, opinions.size());
        assertEquals("Test Opinion", opinions.get(0).getOpinionContent());
    }
